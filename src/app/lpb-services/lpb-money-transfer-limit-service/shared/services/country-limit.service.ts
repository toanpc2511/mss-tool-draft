...
export class CountryLimitService {
  private serviceRoute = `${environment.apiUrl}${API_BASE_URL}/country-limit`;
  ...
  import(request): Observable<DataResponse<any>> {
    return this.http.postUpload<Array<File>>(`${this.serviceRoute}/import`, request);
  }

  saveAll(request: any): Observable<DataResponse<any>> {
    return this.http.post<any>(
      `${this.serviceRoute}/saveAll`,
      request,
      {}
    );
  }
