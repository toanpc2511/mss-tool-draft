...
@Component({
  selector: 'app-uc-guardian',
  templateUrl: './uc-guardian.component.html',
  styleUrls: ['./uc-guardian.component.scss']
})
export class UcGuardianComponent implements OnInit, AfterViewInit {
  ...
  showNewGuardian(): void {
    this.objGuardianCore = null;
    this.inEffect = null; // nếu tạo mới
  ...
  initData(): void {
    ...
    this.guardians = this.guardianList.map(e => ({ ...e }));
  }
  ...
  fillDataVisa(customerInfo: { nationality1Code: string; visaExemption: any; visaIssueDate: moment.MomentInput; visaExpireDate: moment.MomentInput; }): void {
    ...
    if (customerInfo.nationality1Code !== 'VN') {
      this.isShowOptionVisaFree = true;
      // this.isVisaFree = customerInfo.visaExemption ? customerInfo.visaExemption : null;
  ...
  objectTable(): void {
    ...
    this.guardians.forEach((e, index) => {
      if (index === this.prositonEdit) {
        if (e.id) {
          guardian.id = e.id;
          guardian.inEffect = this.inEffect,
            guardian.customer.id = e.customer.id;
          guardian.customer.processId = e.customer.processId;
          guardian.customer.guardianId = e.customer.guardianId;
          guardian.customer.person.id = e.customer.person.id;
          guardian.customer.person.perDocNoList[0].id = e.customer.person.perDocNoList[0].id;
          guardian.customer.person.perDocNoList[0].personId = e.customer.person.perDocNoList[0].personId;
          guardian.customer.person.customerId = e.customer.person.customerId;
          guardian.customer.customerId = e.customer.customerId;
        }
        ...
        this.isShowEffect = true; // hiện thỉ nêu là edit
        isCheckEdit = true;
        this.prositonEdit = -1;
      }
    });
  ...
  taxInputChange(event): void {
    if (event.target.value && event.target.value !== null) {
      this.taxCode = event.target.value.trim();
    }
  }
  /**
   * edit tại row cần sửa trên bảng
   */
  editData(i: number, objSelected?: any): void {
    this.checkDataExistCore(objSelected);
  ...
  checkDataExistCore(obj: any): void {
    this.objGuardianCore = null;
    if (obj && isNotNullOrUndefined(obj.inEffect) && this.lstGuardianCore && this.lstGuardianCore.length > 0) {
      this.objGuardianCore = this.lstGuardianCore.find(item => item.id === obj.id);
    }
