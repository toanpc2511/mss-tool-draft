...
import {
	IDataConnectMqtt,
	IPumpCode,
	PumpCodeManagementService
} from '../pump-code-management.service';
	...
	bindData(dataMqtt: any[]) {
		...
		dataMqtt.forEach((stationMqttData: any[]) => {
			stationMqttData.forEach((pumpPole: IDataConnectMqtt) => {
				const a = this.pumpCodes.find(
					(x) => x.stationCodeChip === pumpPole.station && x.pumpHoseCodeChip === pumpPole.slave
				);
