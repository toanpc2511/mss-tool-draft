	...
	convertToFormArray(data: IRankStock[]): FormArray {
		const controls = data.map((d) => {
			return this.fb.group({
				...
				scoreExportInvoice: [d.scoreExportInvoice, [TValidators.required]],
				scoreNoInvoice: [d.scoreNoInvoice, [TValidators.required]],
				discount: [d.discount]
			});
		});
		...
		this.configurationManagementService
			.updateRankStock({
				rankStockRequests: this.dataSource.value.map((d) => ({
					id: d.id,
					scoreNoInvoice: convertMoney(d.scoreNoInvoice.toString()),
					scoreExportInvoice: convertMoney(d.scoreExportInvoice.toString())
				}))
			})
			.subscribe(
				(res) => {
