	...
	buildForm(): void {
		...
		if (dataProrduct) {
			this.productForm = this.fb.group({
				...
				valueAddedTax: [
					typeof dataProrduct.vat !== 'object'
			...
			this.productForm = this.fb.group({
				code: ['SNL', [Validators.required, TValidators.patternNotWhiteSpace(/^[A-Za-z0-9]*$/)]],
		...
		fromEvent(this.btnSave.nativeElement, 'click')
			...
			.subscribe(() => {
				...
				if (!this.data.product) {
					...
					this.productService.updateProduct(this.data.product.id, valueForm).subscribe(
						() => {
							this.modal.close(true);
						},
						(error: IError) => {
