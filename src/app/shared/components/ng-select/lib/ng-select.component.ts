	...
	implements OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor
{
	...
	@Input() @HostBinding('class.ng-select-taggable') addTag:
		| boolean
		| AddTagFn = false;
	...
	filter(term: string) {
		...
		if (
			this._isTypeahead &&
			(this._validTerm || this.minTermLength === 0)
		) {
		...
		for (const item of this.selectedItems) {
			if (this.bindValue) {
				...
				if (item.children) {
					...
					value = this.itemsList.resolveNested(
						item.value,
						this.bindValue
					);
				}
	...
	private _mergeGlobalConfig(config: NgSelectConfig) {
		...
		this.typeToSearchText =
			this.typeToSearchText || config.typeToSearchText;
