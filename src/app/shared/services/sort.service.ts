  ...
  private sortData(dataSource: Array<T>) {
    if (this.sorting.column) {
      if (this.sorting.direction === 'desc') {
        return orderBy(dataSource, (data) =>
          typeof (data[this.sorting.column]) !== 'number' ? data[this.sorting.column].toLowerCase() : data[this.sorting.column], 'desc');
      }
      return orderBy(dataSource, (data) =>
        typeof (data[this.sorting.column]) !== 'number' ? data[this.sorting.column].toLowerCase() : data[this.sorting.column], 'asc');
    }
    return dataSource;
  }
}
