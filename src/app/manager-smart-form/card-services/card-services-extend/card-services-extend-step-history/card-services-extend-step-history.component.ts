...
@Component({
  selector: 'app-card-services-extend-step-history',
  templateUrl: './card-services-extend-step-history.component.html',
  styleUrls: [
    '../card-services-extend-step-search/card-services-extend-step-search.component.scss',
    './card-services-extend-step-history.component.scss',
  ],
})
export class CardServicesExtendStepHistoryComponent
  implements OnInit, AfterViewInit
{
  ...
  searchHistories(): void {
    ...
    const request: SearchServiceHistoryRequest = {
      ...
      searchType: !!frmValues.serviceStatus
        ...
        : SEARCH_STORIES_TYPES.GDV_ALL,
      sendDateFrom: moment(this.fromDate.getSelectedDate()).format(
        FORMAT_REQUEST_DATE
      ),
      sendDateTo: moment(this.toDate.getSelectedDate()).format(
        FORMAT_REQUEST_DATE
      ),
      page: this.pageIndex,
      size: this.pageSize,
      // khởi tạo 4 thuộc tính của txtSearch bằng ''
      customerCode: '',
      uidValue: '',
      phoneNumber: '',
      cardId: '',
  ...
  exportHistories(): void {
    const sendDateToLast3Months = moment(
      this.preRequest.sendDateTo,
      'YYYY/MM/DD'
    ).subtract({ months: 3 });

    if (
      sendDateToLast3Months.isAfter(
        moment(this.preRequest.sendDateFrom, 'YYYY/MM/DD')
      )
    ) {
