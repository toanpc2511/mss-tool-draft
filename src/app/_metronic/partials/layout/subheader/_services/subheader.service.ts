...
export class SubheaderService implements OnDestroy {
	...
	setDefaultSubheader() {
		this.setSubheaderVersion(
			this.layout.getProp('subheader.layoutVersion')
		);
	}
	...
	private getLinksFromMenu(menu): HTMLAnchorElement[] {
		...
		parentLiElements.forEach((el: any) => {
			const links = Array.from(
				el.getElementsByClassName('menu-link') || []
			);
			if (links && links.length > 0) {
				const aLink = links[0] as any;
				if (
					aLink.href &&
					aLink.href.length &&
					aLink.href.length > 0 &&
		...
		childLiElements.forEach((el: any) => {
			const links = Array.from(
				el.getElementsByClassName('menu-link') || []
			);
			if (links && links.length > 0) {
